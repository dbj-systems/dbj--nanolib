...
#define DBJ_LOG_MAX_LINE_LEN 1024U
	...
	// this is how we decouple log targets
	// users need to provide implementation of a "sink function"
	// and then set it to be used... se bellow
	...
	// note! depends on the current app wide locale
...
#error __STDC_LIB_EXT1__ needs to be defined before using gmtime_s
		...
		if (chk) {
			DBJ_NANO_LOG_FAIL_POLICY("gmtime_s() has failed");
		}
		...
		// value initalization of native array inside std::array aggregate
		timestamp_buffer_type time_stamp_{ {} };
	...
	namespace config
	{
		...
		-----------------------------------------------------------------------------------------------------
		*/
		inline std::array timestamp_functions
		{
		high_precision_timestamp<false>,    /*0*/
		high_precision_timestamp<true>,     /*1*/
		null_timestamp                      /*2*/
		};
