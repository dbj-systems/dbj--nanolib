...
#define DBJ_RETVAL_ERR( SVC_, TYPE_ , CODE_) dbj::nanolib::make_error<TYPE_>( DBJ_STATUS(SVC_, CODE_) )

#define DBJ_RETVAL_FULL( SVC_, VAL_, CODE_) dbj::nanolib::make_full< SVC_ , decltype(VAL_) >( VAL_, DBJ_STATUS(SVC_, CODE_) )

// status part is redundant --> { { value } , { } }
#define DBJ_RETVAL_OK(  VAL_) dbj::nanolib::make_ok< decltype(VAL_) >( VAL_ )
